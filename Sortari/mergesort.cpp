...
template <typename T>
void mergeSortVector(vector<T>& v, int st, int dr) {
    if (st >= dr) return;

    int mid = (st + dr) / 2;

    mergeSortVector(v, st, mid);
    mergeSortVector(v, mid + 1, dr);
...
template <typename T>
void mergeSortArray(T v[], int st, int dr) {
    if (st >= dr) return;

    int mid = (st + dr) / 2;

    mergeSortArray(v, st, mid);
    mergeSortArray(v, mid + 1, dr);
...
bool runTestVector(const string& inputFile, const string& outputFile, long long& duration_ns) {
    ...
    Timer timer;
    if (n > 0) mergeSortVector(v, 0, n - 1);
